...
object Doc {
  ...
  def cachedJavadoc(
    label: String,
    storeFactory: CacheStoreFactory,
    doc: JavaTools
  ): JavaDoc =
    cached(
      storeFactory,
  ...
  private[sbt] def cached(
    ...
    new JavaDoc {
      def run(
        ...
        val cachedDoc = inputChanged(storeFactory make "inputs") {
          (inChanged, in: Inputs) =>
            inputChanged(storeFactory make "output") {
              (outChanged, outputs: List[PlainFileInfo]) =>
        ...
        cachedDoc(inputs)(
          filesInfoToList(exists(outputDirectory.allPaths.get.toSet))
        )
      }
    }
  ...
  private[sbt] def filesInfoToList[A <: FileInfo](
    info: FilesInfo[A]
  ): List[A] =
    info.files.toList sortBy { x =>
      x.file.getAbsolutePath
    }
  private[sbt] val javaSourcesOnly: File => Boolean =
