...
class LookupImpl(
  ...
  private val entry =
    MixedAnalyzingCompiler.classPathLookup(compileConfiguration)

  override def lookupAnalysis(
  ...
  lazy val externalLookup =
    Option(compileConfiguration.incOptions.externalHooks())
  ...
  override def shouldDoIncrementalCompilation(
    ...
    externalLookup.forall(
      _.shouldDoIncrementalCompilation(changedClasses, analysis)
    )
}
