...
private[sbt] object Parser {
  ...
  private def parse(readableName: String)(
    is: InputStream
  ): Either[String, ClassFile] =
  ...
  private def parseImpl(readableName: String, is: InputStream): ClassFile = {
    ...
    new ClassFile {
      ...
      val interfaceNames = array(in.readUnsignedShort())(
        getClassConstantName(in.readUnsignedShort())
      )
      ...
      private def readFieldsOrMethods() =
        array(in.readUnsignedShort())(parseFieldOrMethodInfo())
      private def toUTF8(entryIndex: Int) = {
        val entry = constantPool(entryIndex)
        assume(
          entry.tag == ConstantUTF8,
          "Constant pool entry is not a UTF8 type: " + entryIndex
        )
  ...
  private def array[T: scala.reflect.Manifest](size: Int)(f: => T) =
    Array.tabulate(size)(_ => f)
  ...
  private def getConstant(in: DataInputStream): Constant = {
    ...
    (tag.toInt: @switch) match {
      case ConstantClass | ConstantString =>
        new Constant(tag, in.readUnsignedShort())
      case ConstantField | ConstantMethod | ConstantInterfaceMethod |
        ConstantNameAndType =>
        new Constant(tag, in.readUnsignedShort(), in.readUnsignedShort())
      case ConstantInteger =>
        new Constant(tag, new java.lang.Integer(in.readInt()))
      case ConstantFloat =>
        new Constant(tag, new java.lang.Float(in.readFloat()))
      case ConstantLong => new Constant(tag, new java.lang.Long(in.readLong()))
      case ConstantDouble =>
        new Constant(tag, new java.lang.Double(in.readDouble()))
  ...
  def entryIndex(a: AttributeInfo) = {
    require(
      a.value.length == 2,
      s"Expected two bytes for unsigned value; got: ${a.value.length}"
    )
