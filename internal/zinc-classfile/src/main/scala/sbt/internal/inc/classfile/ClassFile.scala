        ...
        case Constant(ConstantString, nextOffset, _, _) =>
          ...
          nextConstant.value.getOrElse {
            throw new IllegalStateException(
              s"Empty UTF8 value in constant pool: $nextConstant"
            )
          }
        case constant @ Constant((ConstantFloat | ConstantLong |
          ConstantDouble | ConstantInteger),
          _,
          _,
          ref) =>
          ref.getOrElse {
            throw new IllegalStateException(
              s"Empty primitive value in constant pool: $constant"
            )
          }
        case constant =>
          throw new IllegalStateException(
            s"Unsupported ConstantValue type: $constant"
          )
      }
}
...
private[sbt] final case class Constant(
  ...
  def this(tag: Byte, nameIndex: Int, typeIndex: Int) =
    this(tag, nameIndex, typeIndex, None)
...
private[sbt] final case class FieldOrMethodInfo(
  ...
  def isMain =
    isPublic && isStatic && descriptor.exists(_ == "([Ljava/lang/String;)V")
}
private[sbt] final case class AttributeInfo(
  name: Option[String],
  value: Array[Byte]
) {
