...
trait RelationsTextFormat extends FormatCommons {
  ...
  private def stringsDescriptor(
    header: String,
    rels: Relations => Relation[String, String]
  ) =
  ...
  private val allRelations: List[Descriptor[_, _]] = {
    List(
      ...
      stringsDescriptor(
        ...
        _.memberRef.internal
      ),
      stringsDescriptor(
        "member reference external dependencies",
        _.memberRef.external
      ),
      stringsDescriptor(
        "inheritance internal dependencies",
        _.inheritance.internal
      ),
      stringsDescriptor(
        "inheritance external dependencies",
        _.inheritance.external
      ),
      stringsDescriptor(
        "local internal inheritance dependencies",
        _.localInheritance.internal
      ),
      stringsDescriptor(
        "local external inheritance dependencies",
        _.localInheritance.external
      ),
      Descriptor(
        "class names",
        _.classes,
  ...
  protected object RelationsF {
    ...
    def write(out: Writer, relations: Relations): Unit = {
      def writeRelation[A, B](
        relDesc: Descriptor[A, B],
        relations: Relations
      ): Unit = {
        ...
        val toStringOrdA = new Ordering[A] {
          def compare(a: A, b: A) =
            relDesc.keyMapper.write(a) compare relDesc.keyMapper.write(b)
        }
        ...
        rel.forwardMap.toSeq.sortBy(_._1)(toStringOrdA).foreach {
          case (k, vs) =>
            ...
            vs.toSeq.sorted(toStringOrdB) foreach { v =>
              out.write(kStr); out.write(" -> ");
              out.write(relDesc.valueMapper.write(v)); out.write("\n")
            }
        }
      }
    ...
    def read(in: BufferedReader): Relations = {
      def readRelation[A, B](relDesc: Descriptor[A, B]): Relation[A, B] = {
        ...
        val items = readPairs(in)(
          expectedHeader,
          relDesc.keyMapper.read,
          relDesc.valueMapper.read
        ).toIterator
        ...
        def closeEntry(): Unit = {
          if (currentKey != null)
            forward = (currentKey, currentVals.toSet) :: forward
          currentKey = currentItem._1
          currentVals = currentItem._2 :: Nil
        }
        while (items.hasNext) {
          currentItem = items.next()
          if (currentItem._1 == currentKey)
            currentVals = currentItem._2 :: currentVals
          else closeEntry()
        }
    ...
    relations match {
      case p :: bin :: lcn :: mri :: mre :: ii :: ie :: lii :: lie :: cn :: un :: bcn :: Nil =>
        ...
        val internal = InternalDependencies(
          Map(
            DependencyByMemberRef -> mri
              .asInstanceOf[Relation[String, String]],
            DependencyByInheritance -> ii
              .asInstanceOf[Relation[String, String]],
            LocalDependencyByInheritance -> lii
              .asInstanceOf[Relation[String, String]]
          )
        )
        val external = ExternalDependencies(
          Map(
            DependencyByMemberRef -> mre
              .asInstanceOf[Relation[String, String]],
            DependencyByInheritance -> ie
              .asInstanceOf[Relation[String, String]],
            LocalDependencyByInheritance -> lie
              .asInstanceOf[Relation[String, String]]
          )
        )
        Relations.make(
