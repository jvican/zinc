...
private[inc] object FormatTimer {
  ...
  private val printTimings = "true" == System.getProperty(
    "sbt.analysis.debug.timing"
  )
...
trait FormatCommons {
  ...
  protected def writeHeader(out: Writer, header: String): Unit =
    out.write(header + ":\n")

  protected def expectHeader(
    in: BufferedReader,
    ...
    if (header != expectedHeader + ":")
      throw new ReadException(
        expectedHeader,
        if (header == null) "EOF" else header
      )
  }

  protected def writeSize(out: Writer, n: Int): Unit =
    out.write("%d items\n".format(n))
  ...
  protected def writeSeq[T](
    out: Writer
  )(header: String, s: Seq[T], t2s: T => String): Unit = {
    ...
    val m: Map[String, T] =
      s.zipWithIndex.map(x => fmtStr.format(x._2) -> x._1).toMap
  ...
  protected def readSeq[T](in: BufferedReader)(
    expectedHeader: String,
    s2t: String => T
  ): Seq[T] =
    (readPairs(in)(expectedHeader, identity[String], s2t).toSeq
      .sortBy(_._1) map (_._2))

  protected def readPairs[K, V](in: BufferedReader)(
    expectedHeader: String,
    s2k: String => K,
    s2v: String => V
  ) =
  ...
  protected def readMappedPairs[K, V](in: BufferedReader)(
    ...
    def toPair(s: String): (K, V) = {
      ...
      val v =
        s2v(k, if (p == s.length - 4) in.readLine() else s.substring(p + 4))
      (k, v)
    }
