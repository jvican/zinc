...
class TextAnalysisFormat(override val mappers: AnalysisMappers)
  ...
  private implicit val compilationF: Format[Compilation] =
    xsbt.api.CompilationFormat
  ...
  private implicit val companionsFomrat: Format[Companions] =
    xsbt.api.CompanionsFormat
  ...
  private implicit def fileHashFormat: Format[FileHash] =
    asProduct2((file: File, hash: Int) => new FileHash(file, hash))(h =>
      (h.file, h.hash))
  private implicit def seqFormat[T](
    implicit
    optionFormat: Format[T]
  ): Format[Seq[T]] =
    viaSeq[Seq[T], T](x => x)
  private def t2[A1, A2](a1: A1, a2: A2): T2[A1, A2] =
    InterfaceUtil.t2(a1 -> a2)
  ...
  def write(out: Writer, analysis: CompileAnalysis, setup: MiniSetup): Unit = {
    ...
    FormatTimer.time("write relations") {
      RelationsF.write(out, analysis0.relations)
    }
    ...
    FormatTimer.time("write sourceinfos") {
      SourceInfosF.write(out, analysis0.infos)
    }
    FormatTimer.time("write compilations") {
      CompilationsF.write(out, analysis0.compilations)
    }
    out.flush()
  }
  ...
  def read(
    in: BufferedReader,
    companionsStore: CompanionsStore
  ): (CompileAnalysis, MiniSetup) = {
    VersionF.read(in)
    ...
    val compilations = FormatTimer.time("read compilations") {
      CompilationsF.read(in)
    }
  ...
  private[this] object VersionF {
    ...
    def read(in: BufferedReader): Unit = {
      in.readLine() match {
        ...
        case s: String =>
          throw new ReadException("\"format version: <version>\"", s)
  ...
  private[this] object StampsF {
    ...
    def write(out: Writer, stamps: Stamps): Unit = {
      def doWriteMap[V](
        ...
        val pairsToWrite = m.keys.toSeq.sorted map (k =>
          (k, valueMapper.write(k, m(k))))
        writePairs(out)(
          header,
          pairsToWrite,
          keyMapper.write,
    ...
    def read(in: BufferedReader): Stamps = {
      ...
      val products = doReadMap(
        Headers.products,
        mappers.productMapper,
        mappers.productStampMapper
      )
      val sources = doReadMap(
        Headers.sources,
        mappers.sourceMapper,
        mappers.sourceStampMapper
      )
      val binaries = doReadMap(
        Headers.binaries,
        mappers.binaryMapper,
        mappers.binaryStampMapper
      )
  ...
  private[this] object APIsF {
    ...
    def read(
      in: BufferedReader,
      companionsStore: Option[CompanionsStore]
    ): APIs = {
      val internal =
        readMap(in)(Headers.internal, identity[String], stringToAnalyzedClass)
      val external =
        readMap(in)(Headers.external, identity[String], stringToAnalyzedClass)
      ...
      companionsStore match {
        case Some(companionsStore) =>
          ...
          APIs(
            internal map {
              case (k, v) => k -> v.withApi(lzy(companions.get._1(k)))
            },
            external map {
              case (k, v) => k -> v.withApi(lzy(companions.get._2(k)))
            }
          )
  ...
  private[this] object CompanionsF {
    ...
    def read(in: BufferedReader): (Map[String, Companions], Map[String, Companions]) = {
      val internal =
        readMap(in)(Headers.internal, identity[String], stringToCompanions)
      val external =
        readMap(in)(Headers.external, identity[String], stringToCompanions)
      (internal, external)
    }
  }
  ...
  private[this] object MiniSetupF {
    ...
    def write(out: Writer, setup: MiniSetup): Unit = {
      val (mode, outputAsMap) = setup.output match {
        ...
        case m: MultipleOutput =>
          (
            multipleOutputMode,
            m.outputGroups
            .map(x => x.sourceDirectory -> x.outputDirectory)
            .toMap
          )
      }
      ...
      writeSeq(out)(
        ...
        setup.options.javacOptions,
        mappers.javacOptions.write
      )
      writeSeq(out)(
        Headers.compilerVersion,
        setup.compilerVersion :: Nil,
        identity[String]
      )
      writeSeq(out)(
        Headers.compileOrder,
        setup.order.name :: Nil,
        identity[String]
      )
      writeSeq(out)(
        Headers.skipApiStoring,
        setup.storeApis() :: Nil,
        (b: Boolean) => b.toString
      )
      writePairs[String, String](out)(Headers.extra, setup.extra.toList map {
        x =>
          (x.get1, x.get2)
    ...
    def read(in: BufferedReader): MiniSetup = {
      ...
      val outputDirMode =
        readSeq(in)(Headers.outputMode, identity[String]).headOption
      val outputAsMap = readMap(in)(
        Headers.outputDir,
        mappers.sourceDirMapper.read,
        mappers.outputDirMapper.read
      )
      val classpathHash = readSeq(in)(Headers.classpathHash, stringToFileHash) // TODO
      val compileOptions =
        readSeq(in)(Headers.compileOptions, mappers.scalacOptions.read)
      val javacOptions =
        readSeq(in)(Headers.javacOptions, mappers.javacOptions.read)
      val compilerVersion =
        readSeq(in)(Headers.compilerVersion, identity[String]).head
      val compileOrder =
        readSeq(in)(Headers.compileOrder, identity[String]).head
      val skipApiStoring = readSeq(in)(Headers.skipApiStoring, s2b).head
      val extra = readPairs(in)(
        Headers.extra,
        identity[String],
        identity[String]
      ) map {
        ...
        case Some(s) =>
          s match {
            ...
            case `multipleOutputMode` =>
              new MultipleOutput {
                val outputGroups: Array[MultipleOutput.OutputGroup] =
                  outputAsMap.toArray.map {
            ...
            case str: String =>
              throw new ReadException("Unrecognized output mode: " + str)
          }
  ...
  private[this] object ObjectStringifier {
    def objToString[T](o: T)(implicit fmt: sbinary.Format[T]) = {
      ...
      FormatTimer.aggregate("bytes -> base64") {
        Base64.factory().encode(bytes)
      }
    }

    def stringToObj[T](s: String)(implicit fmt: sbinary.Format[T]) = {
      val bytes = FormatTimer.aggregate("base64 -> bytes") {
        Base64.factory().decode(s)
      }
