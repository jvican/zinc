...
class ExportableCache(
  ...
  cleanOutputMode: CleanOutputMode = CleanOutput
)
  ...
  ): Option[(CompileAnalysis, MiniSetup)] = {
    val store =
      FileBasedStore(analysisFile.toFile, createMapper(projectLocation))
  ...
  def exportCache(
    projectLocation: File,
    currentAnalysisStore: AnalysisStore
  ): Option[VerficationResults] = {
    for (
      (currentAnalysis: Analysis, currentSetup) <- currentAnalysisStore
        .get()
    ) yield {
  ...
  protected def cleanOutput(output: File): Unit = {
    if (output.exists()) {
      if (output.isDirectory) {
        ...
        throw new IllegalStateException(
          s"Output file: $output is not a directory"
        )
    }
  }

  protected def importBinaryCache(
  ...
  protected def exportBinaryCache(
