...
private[inc] abstract class IncrementalCommon(
  ...
  @tailrec final def cycle(
    ...
    if (invalidatedRaw.isEmpty && modifiedSrcs.isEmpty)
      ...
      val invalidatedPackageObjects = this.invalidatedPackageObjects(
        invalidatedRaw,
        previous.relations,
        previous.apis
      )
      ...
      val (current, recompiledRecently) = recompileClasses(
        invalidatedClasses,
        modifiedSrcs,
        allSources,
        binaryChanges,
      ...
      if (recompiledRecently == allSources) current
        ...
        val recompiledClasses = invalidatedClasses ++
          modifiedSrcs.flatMap(previous.relations.classNames) ++ modifiedSrcs
          .flatMap(current.relations.classNames)

        val incChanges = changedIncremental(
          recompiledClasses,
        ...
        val classToSourceMapper =
          new ClassToSourceMapper(previous.relations, current.relations)
        ...
        val allInvalidated =
          if (lookup.shouldDoIncrementalCompilation(
  ...
  private[this] def recompileClasses(
    ...
    val invalidatedSourcesForCompilation =
      expand(invalidatedSources, allSources)
    val pruned = Incremental.prune(
    ...
    try {
      ...
      apiChanges foreach {
        ...
        case apiChange: NamesChange =>
          ...
          val apiUnifiedPatch = apiDiff.generateApiDiff(
            src.toString,
            oldApi.api,
            newApi.api,
            contextSize
          ...
          wrappedLog.debug(
            s"Detected a change in a public API ($src):\n$apiUnifiedPatch"
          )
      }
    } catch {
      ...
      case e: LinkageError =>
        log.error(
          "Encountered linkage error while trying to load DiffUtils library."
        )
        log.trace(e)
      case e: Exception =>
        log.error(
  ...
  def shortcutSameClass(a: AnalyzedClass, b: AnalyzedClass): Boolean =
    a.compilationTimestamp() == b
      .compilationTimestamp() && (a.apiHash == b.apiHash)
  ...
  def changedInitial(
    ...
    current: ReadStamps,
    lookup: Lookup
  )(implicit equivS: Equiv[Stamp]): InitialChanges = {
    ...
    val binaryDepChanges = lookup.changedBinaries(previousAnalysis).getOrElse {
      previous.allBinaries
        .filter(
          externalBinaryModified(lookup, previous, current, previousRelations)
        )
        .toSet
    }

    val incrementalExtApiChanges = changedIncremental(
    ...
    val extApiChanges =
      if (lookup.shouldDoIncrementalCompilation(
        incrementalExtApiChanges.allModified.toSet,
        previousAnalysis
      )) incrementalExtApiChanges
  ...
  def transitiveDependencies(
    ...
    val transitivePartial =
      includeInitialCond(initial, transitiveWithInitial, dependsOnClass)
  ...
  def invalidateInitial(
    ...
    val dependentOnRemovedClasses =
      removedClasses.flatMap(previous.memberRef.internal.reverse)
    ...
    val byExtSrcDep = {
      val classNames = invalidateByAllExternal(
        previous,
        changes.external,
        classToSrc.isDefinedInScalaSrc
  ...
  protected def invalidateByExternal(
    relations: Relations,
   ...
   */
  private[this] def includeInitialCond(
    initial: Set[String],
    currentInvalidations: Set[String],
    allDeps: String => Set[String]
  ): Set[String] = {
    ...
    dependsOn => {
      ...
      def entryModified(className: String, classpathEntry: File): Boolean = {
        ...
        if (resolved.getCanonicalPath != dependsOn.getCanonicalPath)
          inv(
            "class " + className + " now provided by " + resolved.getCanonicalPath
          )
        else
        ...
        classNames exists { binaryClassName =>
          ...
          if (lookup.changedClasspathHash.isEmpty)
            lookup.lookupAnalysis(binaryClassName) match {
              case None => false
              case Some(e) =>
                inv(s"shadowing is detected for class $binaryClassName")
            }
          else
            lookup.lookupOnClasspath(binaryClassName) match {
              case None =>
                inv(s"could not find class $binaryClassName on the classpath.")
    ...
    binaryClassName =>
      {
        orEmpty(
          for {
            ...
            className <- analysis.relations.productClassName
  ...
  def orEmpty(o: Option[AnalyzedClass]): AnalyzedClass =
    o getOrElse APIs.emptyAnalyzedClass
  def orTrue(o: Option[Boolean]): Boolean = o getOrElse true

  protected def transitiveDeps[T](nodes: Iterable[T], logging: Boolean = true)(
    dependencies: T => Iterable[T]
  ): Set[T] = {
    val xs = new collection.mutable.HashSet[T]
    def all(from: T, tos: Iterable[T]): Unit =
      tos.foreach(to => visit(from, to))
