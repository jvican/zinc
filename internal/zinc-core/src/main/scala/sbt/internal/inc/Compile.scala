...
object IncrementalCompile {
  ...
  def apply(
    ...
    val current =
      Stamps.initial(Stamp.lastModified, Stamp.hash, Stamp.lastModified)
    ...
    (_: File, binaryClassName: String) =>
      lookup.lookupAnalysis(binaryClassName) flatMap {
        case (analysis: Analysis) =>
          val sourceClassName = analysis.relations.productClassName
...
private final class AnalysisCallback(
  ...
  private[this] val classPublicNameHashes =
    new HashMap[String, Array[NameHash]]
  private[this] val objectPublicNameHashes =
    new HashMap[String, Array[NameHash]]
  ...
  def problem(
    ...
    for (source <- jo2o(pos.sourceFile)) {
      ...
      map.getOrElseUpdate(source, ListBuffer.empty) += Logger.problem(
        category,
        pos,
        msg,
        severity
  ...
  private[this] def externalSourceDependency(
    ...
    val dependency = new ExternalDependency(
      sourceClassName,
      targetBinaryClassName,
      targetClass,
      context
    )
  ...
  def api(sourceFile: File, classApi: ClassLike): Unit = {
    ...
    if (APIUtil.isScalaSourceName(sourceFile.getName) && APIUtil.hasMacro(
      classApi
    )) macroClasses += className
    val shouldMinimize = !Incremental.apiDebug(options)
    val savedClassApi =
      if (shouldMinimize) APIUtil.minimize(classApi) else classApi
    val apiHash: HashAPI.Hash = HashAPI(classApi)
    val nameHashes = (new xsbt.api.NameHashing(options.useOptimizedSealed()))
      .nameHashes(classApi)
  ...
  def getOrNil[A, B](m: collection.Map[A, Seq[B]], a: A): Seq[B] =
  ...
  private def companionsWithHash(
    className: String
  ): (Companions, HashAPI.Hash) = {
    val emptyHash = -1
    lazy val emptyClass = emptyHash -> APIUtil.emptyClassLike(
      className,
      DefinitionType.ClassDef
    )
    lazy val emptyObject = emptyHash -> APIUtil.emptyClassLike(
      className,
      DefinitionType.Module
    )
    val (classApiHash, classApi) = classApis.getOrElse(className, emptyClass)
    val (objectApiHash, objectApi) =
  ...
  private def nameHashesForCompanions(className: String): Array[NameHash] = {
    ...
    (classNameHashes, objectNameHashes) match {
      ...
      case (None, None) =>
        sys.error("Failed to find name hashes for " + className)
    }
  }
    ...
    (base /: srcs) {
      case (a, src) =>
        ...
        val binaryToSrcClassName = (classNames.getOrElse(src, Set.empty) map {
          case (srcClassName, binaryClassName) =>
            (binaryClassName, srcClassName)
        }).toMap
        val nonLocalProds = nonLocalClasses.getOrElse(
          src,
          Nil: Iterable[(File, String)]
        ) map {
            case (classFile, binaryClassName) =>
              ...
              NonLocalProduct(
                ...
                binaryClassName,
                classFile,
                current product classFile
              )
          }

        val internalDeps =
          classesInSrc.flatMap(cls => intSrcDeps.getOrElse(cls, Set.empty))
        val externalDeps =
          classesInSrc.flatMap(cls => extSrcDeps.getOrElse(cls, Set.empty))
        val binDeps =
          binaries.map(d => (d, binaryClassName(d), current binary d))
