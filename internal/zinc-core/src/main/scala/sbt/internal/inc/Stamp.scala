...
object Stamp {
  ...
  def fromString(s: String): Stamp = s match {
    ...
    case lastModifiedPattern(value) =>
      new LastModified(java.lang.Long.parseLong(value))
    case _ =>
      throw new IllegalArgumentException(
        "Unrecognized Stamp string representation: " + s
      )
  }
  ...
  def tryStamp(g: => Stamp): Stamp = try { g } catch {
    case i: IOException => notPresent
  }

  val notPresent = new Exists(false)
  val present = new Exists(true)

  def getStamp(map: Map[File, Stamp], src: File): Stamp =
    map.getOrElse(src, notPresent)
}
...
object Stamps {
  ...
  def merge(stamps: Traversable[Stamps]): Stamps =
    (Stamps.empty /: stamps)(_ ++ _)
}

private class MStamps(
  val products: Map[File, Stamp],
  ...
  def groupBy[K](
    ...
    (for (k <- prod.keySet ++ sourcesMap.keySet ++ bin.keySet)
      yield (k, kStamps(k))).toMap
  }
  ...
  override def equals(other: Any): Boolean = other match {
    case o: MStamps =>
      products == o.products && sources == o.sources && binaries == o.binaries
    case _ => false
  }

  override lazy val hashCode: Int =
    (products :: sources :: binaries :: Nil).hashCode
...
private class InitialStamps(
  ...
  def binary(bin: File): Stamp = synchronized {
    binaries.getOrElseUpdate(bin, binStamp(bin))
  }
}
