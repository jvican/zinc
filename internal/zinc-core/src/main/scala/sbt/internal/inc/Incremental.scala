...
object Incremental {
  ...
  def compile(
    ...
    val initialChanges =
      incremental.changedInitial(sources, previous, current, lookup)
  ...
  @deprecated(
    "Use `IncOptions.relationsDebug` flag to enable debugging of relations.",
    "0.13.2")
  ...
  private[sbt] def prune(invalidatedSrcs: Set[File],
                         previous: CompileAnalysis): Analysis =
  ...
  private[sbt] def prune(invalidatedSrcs: Set[File],
    ...
    val previous = previous0 match { case a: Analysis => a }
    classfileManager.delete(
      invalidatedSrcs.flatMap(previous.relations.products).toArray)
    previous -- invalidatedSrcs
  }

  private[this] def manageClassfiles[T](options: IncOptions)(
      run: ClassFileManager => T): T = {
