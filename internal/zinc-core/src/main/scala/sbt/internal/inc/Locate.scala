...
object Locate {
  ...
  def find[S](
    name: String,
    gets: Stream[String => Either[Boolean, S]]
  ): Either[Boolean, S] =
    if (gets.isEmpty)
      Left(false)
    else
  ...
  def entry(
    ...
    className =>
      entries.collectFirst {
        case (entry, defines) if defines(className) => entry
      }
  }
  def resolve(f: File, className: String): File =
    if (f.isDirectory) classFile(f, className) else f

  def getValue[S](get: File => String => Option[S])(
    entry: File
  ): String => Either[Boolean, S] = {
  ...
  def definesClass(entry: File): DefinesClass =
    ...
    else if (entry.exists && classpath.ClasspathUtilities.isArchive(
      entry,
      ...
      val jar = try { new ZipFile(entry, ZipFile.OPEN_READ) } catch {
        ...
        case e: ZipException =>
          throw new RuntimeException(
            "Error opening zip file: " + entry.getName,
            e
          )
      }
      try {
