...
object Analysis {
  ...
  def summary(a: Analysis): String = {
    ...
    val unreportedCount =
      a.infos.allInfos.values.map(_.unreportedProblems.size).sum
    val sections =
...
private class MAnalysis(
  ...
  def --(sources: Iterable[File]): Analysis = {
    ...
    def keep[T](f: (Relations, T) => Set[_]): T => Boolean =
      f(newRelations, _).nonEmpty

    val classesInSrcs = sources.flatMap(relations.classNames)
    val newAPIs =
      apis.removeInternal(classesInSrcs).filterExt(keep(_ usesExternal _))
    val newStamps =
      stamps.filter(keep(_ produced _), sources, keep(_ usesLibrary _))
  ...
  def addSource(
    ...
    val newStamps = {
      val nonLocalProductStamps =
        nonLocalProducts.foldLeft(stamps.markInternalSource(src, stamp)) {
          case (tmpStamps, nonLocalProduct) =>
            tmpStamps.markProduct(
              nonLocalProduct.classFile,
              nonLocalProduct.classFileStamp
            )
        }
      ...
      val allProductStamps = localProducts.foldLeft(nonLocalProductStamps) {
        case (tmpStamps, localProduct) =>
          tmpStamps.markProduct(
            localProduct.classFile,
            localProduct.classFileStamp
          )
      }
    ...
    val newInternalAPIs = apis.foldLeft(this.apis) {
      case (tmpApis, analyzedClass) =>
        tmpApis.markInternalAPI(analyzedClass.name, analyzedClass)
    }
    ...
    val allProducts = nonLocalProducts.map(_.classFile) ++ localProducts.map(
      _.classFile
    )
    val classes = nonLocalProducts.map(p => p.className -> p.binaryClassName)

    val newRelations = relations.addSource(
      src,
      allProducts,
      classes,
      internalDeps,
