...
/**
  ...
  */
private final class IncrementalNameHashing(log: sbt.util.Logger,
                                           options: IncOptions)
    extends IncrementalCommon(log, options) {

  private val memberRefInvalidator =
    new MemberRefInvalidator(log, options.logRecompileOnMacro())
  ...
  protected def invalidatedPackageObjects(invalidatedClasses: Set[String],
    ...
    transitiveDeps(invalidatedClasses, logging = false)(
      relations.inheritance.internal.reverse) filter {
      _.endsWith(".package")
    }
  }
  ...
  override protected def sameAPI(className: String,
    ...
    if (SameAPI(a, b))
      ...
      val modifiedNames =
        ModifiedNames.compareTwoNameHashes(aNameHashes, bNameHashes)
  ...
  override protected def invalidateByExternal(
      relations: Relations,
    ...
    val invalidationReason =
      memberRefInvalidator.invalidationReason(externalAPIChange)
    log.debug(
    ...
    val byExternalInheritance =
      externalInheritanceR.reverse(modifiedBinaryClassName)
    log.debug(
    ...
    val localInheritance =
      relations.localInheritance.external.reverse(modifiedBinaryClassName)
  ...
  private def invalidateByInheritance(relations: Relations,
                                      modified: String): Set[String] = {
    ...
    log.debug(
      "Invalidated by transitive inheritance dependency: " + transitiveInheritance)
    transitiveInheritance
  }

  private def invalidateByLocalInheritance(relations: Relations,
                                           modified: String): Set[String] = {
    val localInheritanceDeps =
      relations.localInheritance.internal.reverse(modified)
    if (localInheritanceDeps.nonEmpty)
      log.debug(
        s"Invalidate by local inheritance: $modified -> $localInheritanceDeps")
    localInheritanceDeps
  }

  override protected def invalidateClass(
      relations: Relations,
      change: APIChange,
      isScalaClass: String => Boolean): Set[String] = {
    val modifiedClass = change.modifiedClass
    val transitiveInheritance =
      invalidateByInheritance(relations, modifiedClass)
    val localInheritance =
      transitiveInheritance.flatMap(invalidateByLocalInheritance(relations, _))

    val memberRefSrcDeps = relations.memberRef.internal
    val memberRefInvalidation = memberRefInvalidator.get(memberRefSrcDeps,
                                                         relations.names,
                                                         change,
                                                         isScalaClass)
    ...
    def debugMessage: String = {
      if (all.isEmpty) s"Change $change does not affect any class."
        ...
        val byTransitiveInheritance =
          if (transitiveInheritance.nonEmpty)
            s"by transitive inheritance: $transitiveInheritance"
          else ""
        val byLocalInheritance =
          if (localInheritance.nonEmpty)
            s"by local inheritance: $localInheritance"
          else ""
        val byMemberRef =
  ...
  override protected def allDeps(
      relations: Relations): (String) => Set[String] =
