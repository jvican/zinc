...
object ClassToAPI {
  ...
  def toDefinitions0(c: Class[_], cmap: ClassMap): Seq[api.ClassLikeDef] = {
    ...
    val clsDef = new api.ClassLikeDef(
      name,
      acc,
      mods,
      annots,
    ...
    val statDef = new api.ClassLikeDef(
      name,
      acc,
      mods,
      annots,
  ...
  def structure(
    ...
    val methods =
      mergeMap(c, c.getDeclaredMethods, c.getMethods, methodToDef(enclPkg))
    val fields =
      mergeMap(c, c.getDeclaredFields, c.getFields, fieldToDef(c, cf, enclPkg))
    val constructors = mergeMap(
      c,
    ...
    if (!Modifier.isPrivate(c.getModifiers))
      cmap.inherited ++= parentJavaTypes.collect {
        case parent: Class[_] => c -> parent
      }
    val parentTypes = types(parentJavaTypes)
    val instanceStructure = new api.Structure(
      lzyS(parentTypes),
      lzyS(all.declared.toArray),
      lzyS(all.inherited.toArray)
    )
    val staticStructure = new api.Structure(
      lzyEmptyTpeArray,
  ...
  @inline private[this] def lzyS[T <: AnyRef](t: T): xsbti.api.Lazy[T] =
    SafeLazyProxy.strict(t)
  @inline final def lzy[T <: AnyRef](t: => T): xsbti.api.Lazy[T] =
    SafeLazyProxy(t)
  private[this] def lzy[T <: AnyRef](
    t: => T,
    cmap: ClassMap
  ): xsbti.api.Lazy[T] = {
    val s = lzy(t)
  ...
  private def allSuperTypes(t: Type): Seq[Type] = {
    @tailrec def accumulate(t: Type, accum: Seq[Type] = Seq.empty): Seq[Type] =
      t match {
        case c: Class[_] =>
          val (parent, interfaces) =
            (c.getGenericSuperclass, c.getGenericInterfaces)
    ...
    @tailrec def flattenAll(interfaces: Seq[Type], accum: Seq[Type] = Seq.empty): Seq[Type] = {
      if (interfaces.nonEmpty) {
        val raw = interfaces map {
          case p: ParameterizedType => p.getRawType; case i => i
        }
   ...
   * banned characters.
   */
  private def singletonForConstantField(
    c: Class[_],
    field: Field,
    constantValue: AnyRef
  ) =
    new api.Singleton(
      pathFromStrings(
        c.getName.split("\\.").toSeq :+ (field.getName + "$" + returnType(
          field
        ) + "$" + constantValue)
      )
    )
  ...
  def defLike[T <: GenericDeclaration](
    ...
    val ret = retType match {
      case Some(rt) => reference(rt); case None => Empty
    }
    new api.Def(
      name,
      access(mods, enclPkg),
    ...
    if (exceptions.length == 0) emptyAnnotationArray
      ...
      arrayMap(exceptions)(
        t =>
          new api.Annotation(
            Throws,
            Array(new api.AnnotationArgument("value", t.toString))
          )
      )
  ...
  def parameter(
    ...
    new api.MethodParameter(
      ...
      false,
      if (varArgs) api.ParameterModifier.Repeated
  ...
  def merge[T](
    ...
    val (inheritedStatic, inheritedInstance) = splitStatic(
      public filter isInherited
    )
    Defs(
  ...
  def typeParameter[T <: GenericDeclaration](
    tp: TypeVariable[T]
  ): api.TypeParameter =
  ...
  def annotation(a: Annotation): api.Annotation =
    new api.Annotation(
      reference(a.annotationType),
      Array(javaAnnotation(a.toString))
    )
  ...
  def array(tpe: api.Type): api.Type =
    new api.Parameterized(ArrayRef, Array(tpe))
  ...
  def referenceP(t: ParameterizedType): api.Parameterized = {
    ...
    val args =
      if (targs.isEmpty) emptyTypeArray
      else arrayMap(targs)(t => reference(t): api.Type)
  ...
  def reference(t: Type): api.Type =
    t match {
      ...
      case gat: GenericArrayType =>
        array(reference(gat.getGenericComponentType))
  ...
  private[this] def PrimitiveMap =
    PrimitiveNames.map(j => (j, j.capitalize)) :+ ("void" -> "Unit")
  ...
  private[this] def typeParameterTypes[T](
    m: Constructor[T]
  ): Array[TypeVariable[Constructor[T]]] =
  ...
  private[this] def typeParameterTypes(
    m: Method
