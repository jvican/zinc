...
object APIUtil {
  val modifiersToByte = (m: Modifiers) => {
    ...
    (x(isAbstract, 0) | x(isOverride, 1) | x(isFinal, 2) | x(isSealed, 3) | x(
      isImplicit,
      4) | x(isLazy, 5) | x(isMacro, 6)).toByte
  }
  ...
  def minimizeClass(c: ClassLike): ClassLike = {
    val savedAnnotations =
      Discovery.defAnnotations(c.structure, (_: Any) => true).toArray[String]
    val struct =
  ...
  private val emptyStructure =
    new Structure(lzy(Array.empty), lzy(Array.empty), lzy(Array.empty))
