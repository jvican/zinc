...
class Discovery(baseClasses: Set[String], annotations: Set[String]) {
  ...
  def discover(c: ClassLike): Discovered = {
    ...
    val onDefs = Discovery.defAnnotations(c.structure, annotations) ++ c.savedAnnotations
      .filter(annotations)
...
object Discovery {
  ...
  def isMainMethod(d: Definition): Boolean =
    d match {
      case d: Def =>
        d.name == "main" && isPublic(d) && isConcrete(d) && isUnit(
          d.returnType
        ) && isStringArray(d.valueParameters)
      case _ => false
    }
  ...
  def pathName(p: APath, id: String): Option[String] = {
    ...
    cs.last match {
      case _: This =>
        ...
        if (ids.length == cs.length - 1) Some((ids ++ Seq(id)).mkString("."))
        else None
      case _ => None
    }
  }
