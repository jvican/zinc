...
object SameAPI {
  ...
  def separateDefinitions(
    s: Seq[Definition]
  ): (Seq[Definition], Seq[Definition]) =
...
class SameAPI(includePrivate: Boolean, includeParamNames: Boolean) {
  ...
  private[this] val debugEnabled =
    java.lang.Boolean.getBoolean("xsbt.api.debug")
  ...
  def sameDefinitions(
    ...
    val (avalues, atypes) = separateDefinitions(
      filterDefinitions(a, topLevel, includePrivate)
    )
    val (bvalues, btypes) = separateDefinitions(
      filterDefinitions(b, topLevel, includePrivate)
    )
    debug(
      sameDefinitions(byName(avalues), byName(bvalues)),
      "Value definitions differed"
    ) &&
      debug(
        sameDefinitions(byName(atypes), byName(btypes)),
        "Type definitions differed"
      )
  }
  def sameDefinitions(
  ...
  def sameNamedDefinitions(a: List[Definition], b: List[Definition]): Boolean = {
      ...
      a match {
        case adef :: atail =>
          def sameDef(
            ...
            remaining: List[Definition]
          ): Boolean =
            remaining match {
              case Nil =>
                debug(
                  flag = false,
                  "Definition different in new API: \n" + adef.name
                )
              case bdef :: btail =>
                val eq = sameDefinitionContent(adef, bdef)
                if (eq) sameDefs(atail, seen ::: btail)
                else sameDef(bdef :: seen, btail)
            }
          sameDef(Nil, b)
        case Nil => true
      }
    ...
    debug(
      a.length == b.length,
      "\t\tLength differed for " + a.headOption
        .map(_.name)
        .getOrElse("empty")
    ) && sameDefs(a, b)
  }
  ...
  def bitSet(m: Modifiers): immutable.BitSet = {
    ...
    (modifiers foldLeft immutable.BitSet.empty) {
      case (bs, (mod, i)) => if (mod) bs + i else bs
    }
  }
  ...
  def sameAnnotationArguments(
    a: Seq[AnnotationArgument],
    b: Seq[AnnotationArgument]
  ): Boolean =
  ...
  def sameParameterizedDefinition(
  ...
  def sameParameterizedSpecificAPI(
    ...
    (a, b) match {
      ...
      case (ca: ClassLikeDef, cb: ClassLikeDef) =>
        sameClassLikeDefSpecificAPI(ca, cb)
      case (ta: TypeAlias, tb: TypeAlias) => sameAliasSpecificAPI(ta, tb)
      case (ta: TypeDeclaration, tb: TypeDeclaration) =>
        sameDeclarationSpecificAPI(ta, tb)
      case _ => false
    }
  ...
  def sameDeclarationSpecificAPI(
    ...
    debug(
      ...
      debug(
        sameType(a.upperBound, b.upperBound),
  ...
  def sameParameters(
    a: Seq[MethodParameter],
    b: Seq[MethodParameter]
  ): Boolean =
  ...
  def sameTypeDirect(a: Type, b: Type): Boolean = {
    (a, b) match {
      ...
      case (sa: Singleton, sb: Singleton) =>
        debug(sameSingleton(sa, sb), "Different singleton")
      case (ca: Constant, cb: Constant) =>
        debug(
          sameConstantType(ca, cb),
          "Different constant types: " + DefaultShowAPI(ca) + " and " + DefaultShowAPI(
            cb
          )
        )
  ...
  def sameStrings(
    a: scala.collection.Set[String],
    b: scala.collection.Set[String]
  ): Boolean =
    a == b
