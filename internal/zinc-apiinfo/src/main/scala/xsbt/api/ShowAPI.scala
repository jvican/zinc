...
object ShowAPI {
  ...
  private def truncateDecls(
      decls: Array[ClassDefinition]): Array[ClassDefinition] =
  ...
  def showDefinition(d: Definition)(implicit nesting: Int): String = d match {
    ...
    case d: Def =>
      showPolyDef(d, "def") + showValueParams(d.valueParameters) + ": " + showType(
        d.returnType)
  ...
  def showType(t: Type)(implicit nesting: Int): String = t match {
    ...
    case a: Annotated =>
      showAnnotations(a.annotations) + " " + showType(a.baseType)
    case s: Structure =>
  ...
  private def showMonoDef(d: Definition, label: String)(
  ...
  private def showTypeParameters(tps: Seq[TypeParameter])(
      implicit nesting: Int): String =
    if (tps.isEmpty) ""
    else tps.map(showTypeParameter).mkString("[", ", ", "]")

  private def showTypeParameter(tp: TypeParameter)(
      implicit nesting: Int): String =
  ...
  private def showBounds(lower: Type, upper: Type)(
      implicit nesting: Int): String =
    ">: " + showType(lower) + " <: " + showType(upper)

  private def showValueParams(ps: Seq[ParameterList])(
      implicit nesting: Int): String =
    ps.map(
        pl =>
          pl.parameters
            .map(mp =>
              mp.name + ": " + showParameterModifier(
                showType(mp.tpe),
                mp.modifier) + (if (mp.hasDefault) "= ..." else ""))
            .mkString(if (pl.isImplicit) "(implicit " else "(", ", ", ")"))
      .mkString("")

  private def showParameterModifier(base: String,
                                    pm: ParameterModifier): String = pm match {
  ...
  private def showNestedTypeParameter(tp: TypeParameter)(
      implicit nesting: Int) = showTypeParameter(tp)(nesting - 1)
  private def showNestedTypeParameters(tps: Seq[TypeParameter])(
      implicit nesting: Int) = showTypeParameters(tps)(nesting - 1)
