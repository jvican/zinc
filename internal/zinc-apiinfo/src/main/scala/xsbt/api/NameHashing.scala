...
/**
  ...
  */
class NameHashing(
    optimizedSealed: Boolean = IncOptionsUtil.defaultUseOptimizedSealed()) {
  ...
  def nameHashes(classApi: ClassLike): Array[NameHash] = {
    ...
    val (regularDefs, implicitDefs) =
      apiPublicDefs.partition(deff => !deff.modifiers.isImplicit)
  ...
  private def nameHashesForDefinitions(
    ...
    val groupedBySimpleName =
      defs.groupBy(locatedDef => localName(locatedDef.name))
    val hashes = groupedBySimpleName.mapValues(
      hashLocatedDefinitions(_, location, includeSealedChildren))
    hashes.toIterable.map({
      case (name: String, hash: Int) => new NameHash(name, useScope, hash)
    })(collection.breakOut)
  }
...
object NameHashing {
  def merge(nm1: Array[NameHash], nm2: Array[NameHash]): Array[NameHash] = {
    ...
    val m: Map[(String, UseScope), Int] = Map(
      nm1.map(nh => (nh.name, nh.scope) -> nh.hash): _*)
    for (nh <- nm2) {
    ...
    m.map { case ((name, scope), hash) => new NameHash(name, scope, hash) }(
      collection.breakOut)
  }

  private case class LocatedDefinition(location: Location,
