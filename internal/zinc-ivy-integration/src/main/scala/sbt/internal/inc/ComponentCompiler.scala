...
private[sbt] object ComponentCompiler {
  ...
  lazy val incrementalVersion = {
    ...
    val propertiesStream = getClass
      .getResource("/incrementalcompiler.version.properties")
      .openStream
    try { properties.load(propertiesStream) } finally {
 ...
 */
private[inc] class IvyComponentCompiler(
  ...
  private def getModule(moduleID: ModuleID): ivySbt.Module = {
    ...
    val dummyID =
      ModuleID(sbtOrgTemp, modulePrefixTemp + sha1, moduleID.revision)
  ...
  private def update(
    ...
    buffered.info(
      s"Attempting to fetch ${dependenciesNames(module)}. This operation may fail."
    )
    IvyActions.updateEither(
      ...
      buffered
        ...
        case Left(unresolvedWarning) =>
          buffered.debug(
            s"Couldn't retrieve module ${dependenciesNames(module)}."
          )
