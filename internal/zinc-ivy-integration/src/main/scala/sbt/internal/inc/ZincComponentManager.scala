...
class ZincComponentManager(
  ...
  private def lockLocalCache[T](action: => T): T =
    lock(provider.lockFile)(action)
  ...
  def file(id: String)(ifMissing: IfMissing): File =
    files(id)(ifMissing).toList match {
      ...
      case xs =>
        invalid(
          "Expected single file for component '" + id + "', found: " + xs
            .mkString(", ")
        )
    }
...
class InvalidComponent(msg: String, cause: Throwable)
...
object IfMissing {
  ...
  def define(useSecondaryCache: Boolean, f: => Unit): IfMissing =
    new Define(useSecondaryCache, f)
  object Fail extends IfMissing
  final class Define(val useSecondaryCache: Boolean, define: => Unit)
    extends IfMissing {
    def run(): Unit = define
  }
}
