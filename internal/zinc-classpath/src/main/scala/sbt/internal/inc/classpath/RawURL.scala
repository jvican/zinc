...
object RawURL {
  ...
  private[this] final class RawStreamHandler(value: => InputStream)
...
trait FixedResources extends ClassLoader {
  ...
  override def findResource(s: String): URL =
    resourceURL.getOrElse(s, super.findResource(s))
  ...
  override def findResources(s: String): Enumeration[URL] = {
    ...
    resourceURL.get(s) match {
      case Some(url) =>
        new DualEnumeration(enumeration(singletonList(url)), sup)
      case None => sup
    }
  }
}
