...
final class ClasspathFilter(
  ...
  private[this] val directories: Seq[File] =
    classpath.toSeq.filter(_.isDirectory)
  ...
  private[this] def onClasspath(src: URL): Boolean =
    (src eq null) || (
      IO.urlAsFile(src) match {
        case Some(f) =>
          classpath(f) || directories.exists(dir =>
            IO.relativize(dir, f).isDefined)
        case None => false
      }
    )
  ...
  @tailrec private[this] def includeLoader(
    c: ClassLoader,
    base: ClassLoader
  ): Boolean =
    (base ne null) && (
 ...
 */
final class FilteredLoader(parent: ClassLoader, filter: ClassFilter)
  extends ClassLoader(parent) {
  require(parent != null) // included because a null parent is legitimate in Java
...
abstract class PackageFilter(packages: Iterable[String]) extends ClassFilter {
  ...
  protected final def matches(className: String): Boolean =
 ...
 */
final class ExcludePackagesFilter(exclude: Iterable[String])
  extends PackageFilter(exclude) {
 ...
 */
final class IncludePackagesFilter(include: Iterable[String])
  extends PackageFilter(include) {
  def include(className: String): Boolean = matches(className)
}
...
trait NativeCopyLoader extends ClassLoader {
  ...
  private[this] def relativeLibrary(mappedName: String)(
    base: File
  ): Seq[File] = {
