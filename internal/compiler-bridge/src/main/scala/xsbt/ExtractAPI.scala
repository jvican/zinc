...
class ExtractAPI[GlobalType <: Global](
  ...
  private[this] val classLikeCache =
    new HashMap[(Symbol, Symbol), xsbti.api.ClassLikeDef]
  ...
  private def thisPath(sym: Symbol) =
    path(pathComponents(sym, Constants.thisPath :: Nil))
  private def path(components: List[PathComponent]) =
    new xsbti.api.Path(components.toArray[PathComponent])
  private def pathComponents(
    sym: Symbol,
    postfix: List[PathComponent]
  ): List[PathComponent] = {
    if (sym == NoSymbol || sym.isRoot || sym.isEmptyPackageClass || sym.isRootPackage)
      postfix
    else
      pathComponents(sym.owner, new xsbti.api.Id(simpleName(sym)) :: postfix)
  }
  ...
  private def projectionType(in: Symbol, pre: Type, sym: Symbol) = {
    if (pre == NoPrefix) {
      if (sym.isLocalClass || sym.isRoot || sym.isRootPackage)
        Constants.emptyType
  ...
  private def mkAnnotations(
    ...
    staticAnnotations(as).toArray.map { a =>
      new xsbti.api.Annotation(
        ...
        if (a.assocs.isEmpty)
          Array(
          new xsbti.api.AnnotationArgument(
            "",
            a.args.mkString("(", ",", ")")
          )
          ...
          a.assocs
            .map {
              case (name, value) =>
                new xsbti.api.AnnotationArgument(name.toString, value.toString)
            }
  ...
  private def annotations(in: Symbol, s: Symbol): Array[xsbti.api.Annotation] =
    enteringPhase(currentRun.typerPhase) {
      ...
      val associated = List(
        b,
        b.getterIn(b.enclClass),
        b.setterIn(b.enclClass)
      ).filter(_ != NoSymbol)
      associated
        .flatMap(ss => mkAnnotations(in, ss.annotations))
        .distinct
        .toArray
    }

  private def viewer(s: Symbol) =
    (if (s.isModule) s.moduleClass else s).thisType
  ...
  private def defDef(in: Symbol, s: Symbol): xsbti.api.Def = {
    def build(
      ...
      def parameterList(syms: List[Symbol]): xsbti.api.ParameterList = {
        val isImplicitList = syms match {
          case head :: _ => isImplicit(head); case _ => false
        }
        new xsbti.api.ParameterList(
          syms.map(parameterS).toArray,
          isImplicitList
        )
      }
      t match {
        ...
        case MethodType(params, resultType) =>
          build(
            resultType,
            typeParams,
            parameterList(params) :: valueParameters
          )
  ...
  private def fieldDef[T](
    ...
    s: Symbol,
    keepConst: Boolean,
    create: (String, xsbti.api.Access, xsbti.api.Modifiers, Array[xsbti.api.Annotation], xsbti.api.Type) => T
  ): T = {
  ...
  private def typeDef(in: Symbol, s: Symbol): xsbti.api.TypeMember = {
    val (typeParams, tpe) =
      viewer(in).memberInfo(s) match {
        case PolyType(typeParams0, base) =>
          (typeParameters(in, typeParams0), base)
  ...
  private def structureWithInherited(
    info: Type,
    s: Symbol
  ): xsbti.api.Structure =
    structureCache.getOrElseUpdate(s, mkStructureWithInherited(info, s))

  private def removeConstructors(ds: List[Symbol]): List[Symbol] = ds filter {
    !_.isConstructor
  }
   ...
   */
  private def mkStructureWithInherited(
    info: Type,
    s: Symbol
  ): xsbti.api.Structure = {
  ...
  def linearizedAncestorTypes(info: Type): List[Type] =
    info.baseClasses.tail.map(info.baseType)
  ...
  private def processDefinitions(
    in: Symbol,
    defs: List[Symbol]
  ): Array[xsbti.api.ClassDefinition] =
  ...
  private def definition(
    in: Symbol,
    sym: Symbol
  ): Option[xsbti.api.ClassDefinition] = {
    def mkVar =
      Some(
        fieldDef(in, sym, keepConst = false, new xsbti.api.Var(_, _, _, _, _))
      )
    def mkVal =
      Some(
        fieldDef(in, sym, keepConst = true, new xsbti.api.Val(_, _, _, _, _))
      )
    if (isClass(sym))
  ...
  private def ignoreClass(sym: Symbol): Boolean =
    sym.isLocalClass || sym.isAnonymousClass || sym.fullName.endsWith(
      tpnme.LOCAL_CHILD.toString
    )
  ...
  private def makeType(in: Symbol, t: Type): xsbti.api.Type = {
    ...
    dealiased match {
      ...
      case ConstantType(constant) =>
        new xsbti.api.Constant(
          processType(in, constant.tpe),
      ...
      case TypeRef(pre, sym, Nil) if sym.isRefinementClass =>
        ...
        val withoutRecursiveRefs =
          new SuppressSymbolRef(sym).mapOver(unrolling)
      ...
      case at: AnnotatedType =>
        at.annotations match {
          ...
          case annots =>
            new xsbti.api.Annotated(
              processType(in, at.underlying),
              mkAnnotations(in, annots)
            )
        }
      ...
      case PolyType(typeParams, resultType) =>
        new xsbti.api.Polymorphic(
          processType(in, resultType),
      ...
      case NullaryMethodType(_) =>
        ...
        Constants.emptyType
      case _ =>
        warning("sbt-api: Unhandled type " + t.getClass + " : " + t);
        Constants.emptyType
    }
  }
  ...
  private def typeParameter(in: Symbol, s: Symbol): xsbti.api.TypeParameter = {
    ...
    val variance =
      if (varianceInt < 0) Contravariant
      else if (varianceInt > 0) Covariant
      else Invariant
    viewer(in).memberInfo(s) match {
    ...
    existentialRenamings.renaming(s) match {
      case Some(rename) =>
        ...
        if (settings.debug.value)
          log(
            "Renaming existential type variable " + s.fullName + " to " + rename
          )
        rename
      case None =>
        s.fullName
  ...
  private def mkClassLike(in: Symbol, c: Symbol): ClassLikeDef = {
    ...
    val childrenOfSealedClass =
      sort(sym.children.toArray).map(c => processType(c, c.tpe))
